...
var FORBID_DEPRECATE = process$1 && process$1.env && process$1.env.FORBID_DEPRECATIONS;
  ...
  createClass(Text, [{
    ...
    value: function render() {
      ...
      var decs = decorations.filter(function (d) {
        ...
        var path = document.assertPathByKey(key);
        if (slate.PathUtils.compare(path, startPath) === -1) return false;

        // If the node's path is after the end path, ignore it.
        if (slate.PathUtils.compare(path, endPath) === 1) return false;
...
function AfterPlugin() {
  ...
  function onInput(event, change, editor) {
    ...
    var entire = selection.moveAnchorTo(point.key, start).moveFocusTo(point.key, end).normalize(document);
  ...
  function onKeyDown(event, change, editor) {
    ...
    if (index$4.isSplitBlock(event) && !IS_IOS) {
      return value.isInVoid ? change.collapseToStartOfNextText() : change.splitBlock();
    }

    if (index$4.isDeleteCharBackward(event) && !IS_IOS) {
      return change.deleteCharBackward();
    }

    if (index$4.isDeleteCharForward(event) && !IS_IOS) {
      return change.deleteCharForward();
    }

    if (index$4.isDeleteLineBackward(event)) {
      return change.deleteLineBackward();
    }

    if (index$4.isDeleteLineForward(event)) {
      return change.deleteLineForward();
    }

    if (index$4.isDeleteWordBackward(event)) {
      return change.deleteWordBackward();
    }

    if (index$4.isDeleteWordForward(event)) {
      return change.deleteWordForward();
    }

    if (index$4.isRedo(event)) {
      return change.redo();
    }

    if (index$4.isUndo(event)) {
      return change.undo();
    }
    ...
    if (index$4.isCollapseLineBackward(event)) {
      event.preventDefault();
      return change.collapseLineBackward();
    }

    if (index$4.isCollapseLineForward(event)) {
      event.preventDefault();
      return change.collapseLineForward();
    }

    if (index$4.isExtendLineBackward(event)) {
      event.preventDefault();
      return change.extendLineBackward();
    }

    if (index$4.isExtendLineForward(event)) {
      event.preventDefault();
    ...
    if (index$4.isCollapseCharBackward(event)) {
      var document = value.document,
    ...
    if (index$4.isCollapseCharForward(event)) {
    ...
    if (index$4.isExtendCharBackward(event)) {
    ...
    if (index$4.isExtendCharForward(event)) {
...
function BeforePlugin() {
  ...
  function onKeyDown(event, change, editor) {
    ...
    if (isComposing) {
      if (index$4.isComposing(event)) event.preventDefault();
      return true;
    }
    ...
    if (index$4.isContentEditable(event) && !IS_IOS) {
      event.preventDefault();
    }
