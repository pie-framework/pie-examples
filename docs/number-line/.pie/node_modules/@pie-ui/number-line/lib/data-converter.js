'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});
const lineIsSwitched = exports.lineIsSwitched = line => {
  let position = line.position;

  return position.left > position.right;
};

const switchGraphLine = exports.switchGraphLine = line => {
  let position = line.position;


  if (position.left < position.right) {
    return line;
  }

  let newRightPoint = line.leftPoint,
      newLeftPoint = line.rightPoint;


  return {
    leftPoint: newLeftPoint,
    rightPoint: newRightPoint,
    position: switchPosition(position),
    type: 'line'
  };
};

const switchPosition = exports.switchPosition = p => {
  let newRight = p.left,
      newLeft = p.right;

  return { left: newLeft, right: newRight };
};

const toSessionFormat = exports.toSessionFormat = gf => {
  if (gf.type === 'point') {
    return {
      type: 'point',
      pointType: gf.pointType,
      domainPosition: gf.position
    };
  } else if (gf.type === 'line') {
    return {
      type: 'line',
      size: gf.position.right - gf.position.left,
      domainPosition: gf.position.left,
      leftPoint: gf.leftPoint,
      rightPoint: gf.rightPoint
    };
  } else if (gf.type === 'ray') {
    return {
      type: 'ray',
      domainPosition: gf.position,
      pointType: gf.pointType,
      direction: gf.direction
    };
  }
};

const toGraphFormat = exports.toGraphFormat = sf => {
  if (sf.type === 'point') {
    return {
      type: 'point',
      pointType: sf.pointType,
      position: sf.domainPosition
    };
  } else if (sf.type === 'line') {
    return {
      type: 'line',
      position: {
        left: sf.domainPosition,
        right: sf.domainPosition + sf.size
      },
      leftPoint: sf.leftPoint,
      rightPoint: sf.rightPoint
    };
  } else if (sf.type === 'ray') {
    return {
      type: 'ray',
      position: sf.domainPosition,
      pointType: sf.pointType,
      direction: sf.direction
    };
  }
};