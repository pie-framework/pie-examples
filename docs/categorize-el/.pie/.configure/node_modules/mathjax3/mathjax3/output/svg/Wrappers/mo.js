...
var SVGmo = (function (_super) {
    ...
    SVGmo.prototype.addExtV = function (n, H, D, T, B, W) {
        ...
        var s = (H + D) / (h + d);
        this.addGlyph(n, 0, 0);
        var glyph = adaptor.lastChild(this.element);
        var scale = 'scale(1 ' + this.jax.fixed(s) + ')';
        var translate = 'translate(' + this.fixed((W - w) / 2) + ' ' + this.fixed(d * s - D) + ')';
        adaptor.setAttribute(glyph, 'transform', translate + ' ' + scale);
        var inset = 'inset(' + [this.fixed((B - VFUZZ) / s), 0, this.fixed((T - VFUZZ) / s), 0].join(' ') + ')';
    ...
    SVGmo.prototype.addExtH = function (n, W, L, R, x) {
        ...
        if (!n)
            return;
        var _a = __read(this.getChar(n), 3), h = _a[0], d = _a[1], cw = _a[2];
        var w = cw + 2 * HBEARING;
        var s = 2 * (W / w);
        var scale = 'scale(' + this.jax.fixed(s) + ', 1)';
        var inset = [0, this.fixed((Math.max(0, R - HFUZZ) + W / 2) / s),
